...
Shark::Shark(QString animationName, Mode mode, void (*callback)()):
    Sprite(animationName, mode, callback)
{
    m_tweener = tweeny::from<float, float>(.0f, .0f).to(.0f, .0f).during(0);
    m_p_move = std::bind(&Shark::move, this, std::placeholders::_1, std::placeholders::_2);
    double width = boundingRect().width();
    double height = boundingRect().height();
    setOffset(-width/2, -height/2);
    //qDebug()<< width << this->offset().x();
}

tweeny::tween<float, float>& Shark::getTweener()
{
    return m_tweener;
}

void Shark::move(int x, int y)
{
    //tween::Tweener& tweener = game.getTweener();
    ...
    float speed = 0.12;
    int during = static_cast<int>(sqrt(pow(x2-x1, 2) + pow(y2-y1, 2)) / speed);
...
void Shark::setDirection(float x1, float y1, float x2, float y2)
{
    resetTransform();
    setRotation(0);
